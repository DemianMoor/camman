...
The reporting surfaces already counted any conversion as a sale — [`lib/keitaro/poll.ts`](../lib/keitaro/poll.ts) used to do `agg.sales += 1` per `conversions/log` row (since Phase 3 Task 3 it folds CLICKS only; the conversion columns of `keitaro_stage_results` are the ledger's projection, written by [`lib/keitaro/stage-day-conversions.ts`](../lib/keitaro/stage-day-conversions.ts), which counts the same Keitaro types per ledger row), [`lib/reporting/rollup.ts`](../lib/reporting/rollup.ts) uses `(converted_at IS NOT NULL)`. The targeting side did not, so **the same 854 conversions read as 887 "Sales" on every report and 2 buyers in segments**, and `Clickers excl Buyers` excluded 2 people instead of 837 — i.e. 825 known buyers were being messaged as non-buyers.
...
The generalisation: **any job that derives a table from another and deletes what it cannot explain needs a coverage bound, not just a correctness proof.** "The ledger doesn't explain this row" and "the ledger hasn't been loaded that far back" look identical to the SQL; only a floor tells them apart. And the ordering is part of the fix, not a nicety — the backfill runs on prod BEFORE the projecting code is live (Task 8's precondition list); the floor and the refusal are the net if that order slips.
