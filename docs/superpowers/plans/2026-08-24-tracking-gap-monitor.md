...
- **NO TOP-LEVEL `await` IN SCRIPTS.** `package.json` has no `"type": "module"`, so `tsx` compiles scripts as CJS and top-level `await` dies with `esbuild: Top-level await is currently not supported with the "cjs" output format`. Measured across `scripts/`: **256 files use an `async function main() { … } main().catch(…)` wrapper; zero use top-level `await`.** The script bodies shown in Tasks 4 and 5 below are written flat for readability — wrap each one in `main()` and end with `main().catch((err) => { console.error(err); process.exit(1); });`, keeping `import "./_env-preload";` as the first import at module scope.
- Scripts that touch the database must `import "./_env-preload";` as their **first** import. ESM hoisting means an inline `config({ path })` runs after `@/db/client` initializes, producing `password authentication failed for user "dimat"`.
