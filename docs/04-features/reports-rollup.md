...
- **Hourly:** `getHourlyReport()` — **user-activity time**, bucketed by hour-of-day (0–23) **summed across the selected date range** (takes a from/to range like the other tabs, 2026-07-20), from internal event tables: clicks by `clicks.clicked_at`, redirects by `offer_reached_at`, sales/revenue by `converted_at`, opt-outs by `opt_out_attributions.created_at`. A pinned **Manual** row (sorts first) rolls up manual-campaign results with no per-event time (manual sales by ledger date + manual-campaign opt-outs) over the range. Hourly deliberately does NOT equal Overview's Keitaro click count (internal ≠ Keitaro) — it answers "when do users engage", per operator request.
