...
export async function POST(
  ...
  if (!can(role, "provider_short_codes.restore")) {
  ...
  // Restore always returns the short code to 'active', regardless of its
  // pre-archive status.
  const updated = await db
    ...
    .where(
      and(
        ...
        eq(provider_short_codes.status, "archived"),
      ),
    )
    .returning();
