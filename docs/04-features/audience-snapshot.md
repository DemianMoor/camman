...
### Recipe composition
1. Per-segment audience clauses (`buildSegmentAudienceClause`, see [audience-segments.md](audience-segments.md)) are **INTERSECT'd** across `audience_segment_ids` (the **include** segment side) — a contact must be in **every** selected segment; direct `contact_contact_groups` members for `audience_contact_group_ids` are UNION'd into the group side. The two dimensions then **INTERSECT** when both are populated — a contact must be in every selected segment **AND** a selected group — yielding the positive base `P`. When only one dimension is filled, that side stands alone (the empty dimension is ignored, not treated as "match nothing"). Composition lives in `buildAudienceSourceClause`; the preview path applies the same rule via a `membership_ok` flag (with a `segments_matched = <n>` test mirroring the INTERSECT chain) so it can still report each side's pre-intersection contribution.

   **Segments AND together (changed 2026-08-17; was UNION).** Each additional include-segment can only *narrow* the audience. The old UNION broke on "filter-shaped" segments — e.g. a segment whose only rule is `in_use_in_campaign_last_period` `is_not`, which matches nearly the whole org. OR-ing that in *grew* a 44K audience to 508K instead of trimming it, and because `buildAudienceSourceClause` is shared with `snapshotAudience`, activating such a campaign would have frozen and sent to the inflated pool — not just displayed it. To OR two audiences together, model it as rules inside **one** segment (rule combinator `or`); that layer still unions. `previewAudience` and `buildAudienceSourceClause` are separate implementations of this rule and **must stay in lockstep** — if they drift, the preview stops predicting what activation freezes ([`scripts/test-segment-intersect-and-optout.ts`](../../scripts/test-segment-intersect-and-optout.ts) asserts both paths land on the same count).
1b. **Per-segment exclude (migration 0114).** `audience_exclude_segment_ids` holds segments whose members are **subtracted** from the positive base: `final = P EXCEPT (union of exclude segments)`. A segment is include XOR exclude (disjoint, enforced by the validator). Exclude is inert only when there is no positive base (no group and no include segment). The exclude union is evaluated against the group universe when a group is present (final ⊆ group — same `restrictUniverse` perf lever as the include side). `previewAudience` reports `excluded_by_segments` (contacts in `P` removed by an exclude segment) so the UI can show "N excluded by segment". Existing campaigns default to `'{}'` (no excludes) — behavior unchanged.
...
Five optimizations keep this fast even at ~750K contacts:
...
4. **Prior-offer exclusion is its own statement (snapshot path).** Content-dedup LAYER 3 (`exclude_prior_offer_contacts`) is applied by `DELETE … WHERE EXISTS (offer_exposures …)` against the *materialized + ANALYZEd* `audience_qualified` temp table — **not** as an `oe_set` anti-join inside the qualifier. Folded in, two independent misestimates line up: the qualifier's outer estimate collapses to `rows=1` (a chain of anti-joins over the candidate temp table), and `offer_id` for a newer/small offer is missing from the column's MCV list so `offer_exposures` is estimated at 1 row as well. The planner then picks a **Nested Loop Anti Join** and re-scans `offer_exposures` once per candidate. Measured 2026-07-31 on a real recipe (36K candidates, 23K qualified, offer with 5K exposures): **23,139 loops × 5,000 rows = 115.7M heap fetches, ~84s** — past the route's `maxDuration = 60`, so campaign create returned Vercel's `Task timed out after 60 seconds` and the transaction rolled back (four burned campaign IDs that day). Splitting it out gives the planner the real cardinality of the qualified set, so it hash-anti-joins: **67.5s → 4.2s, byte-identical rows out.** The failure is *data-shape dependent* — the same recipe planned fine hours earlier and campaigns on high-frequency offers never tripped it, which is why it surfaced as an intermittent "sometimes I can't create a campaign". `previewAudience` aggregates over a different shape and does **not** hit this (measured 3.9–5.9s on the same recipes), so it keeps the inline `oe_set` join.
5. **Batched per-stage counts (stages-list endpoint).** `GET /api/campaigns/[campaignId]/stages` needs `audience_count` for every stage. Computing it one query per stage was an N+1 that dominated the page (the slowest in the app). `computeStageAudienceCountsBatch` / `…ForDraft` collapse all non-lane stages into a **single** pass — the pool/source is scanned once and per-stage filters + the split bucket (`ROW_NUMBER() PARTITION BY stage_id ORDER BY contact_id`) are applied via conditional aggregation. Behavioral-lane stages keep their own per-lane `countStageRecipients` (live tier + aliveness) unchanged. Proven numerically identical to the per-stage functions across all real campaigns (active, mid-send, lane, and forced-draft cases) before shipping. The draft batch CTE is `MATERIALIZED`: without it the planner re-evaluated the expensive segment-rule source once *per stage inside one statement* (nested loop), which tripped `statement_timeout` where the old N-separate-statements path did not — `MATERIALIZED` forces one source evaluation. The route sets `maxDuration = 30`.
