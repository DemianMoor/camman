...
- **Total Cost auto-derivation** (migration `0081`, [`lib/stages/total-cost.ts`](../../lib/stages/total-cost.ts)): a stage's `total_cost` defaults to `cost_per_sms × (sends + opt_out_count)` — the assigned provider phone's per-SMS rate times sends **plus** opt-out replies (STOPs are billed like sends). **`sends = GREATEST(sms_count, accepted stage_sends)`**: API/tracked stages dispatch one `stage_sends` row per recipient and leave `sms_count` at 0, so the count of provider-accepted rows (`status='sent'`, the same number the "Submitted / accepted by TextHub" badge shows) is used; manual/CSV stages have no `stage_sends` and carry the count in `sms_count`. It's recomputed on every write that moves those inputs: the manual-results save, the opt-out poller after a STOP bumps `opt_out_count`, and the stage PATCH when `provider_phone_id` changes. `total_cost_manual` is the override flag: the manual-results form's **Auto-calculate total cost** switch turns it on/off, and a CSV import that carries a real provider cost sets it (the imported figure is authoritative and replaces a previously auto-derived value). When the flag is true the auto formula never touches `total_cost`.
