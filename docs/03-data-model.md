...
| `send_circuit_events` | `provider_id`, `event` (`paused`/`resumed`/`api_send_enabled`/`api_send_disabled`), `reason`, `actor_user_id` | append-only per-provider **send-posture** audit. Migration 0131 widened the CHECK from the original two values so the `supports_api_send` go-live gate is audited here too, rather than in a second table — both answer "may this provider send right now, who changed that, and when". Distinct verbs so the breaker and the gate can never be confused when reading history back. Before 0131 the gate had NO audit trail at all and `sms_providers` has no `updated_at`, which is why a silent reversion could not be attributed (ClickUp 869ehjwtf) |
