...
**Total Sent is a HYBRID read (2026-09-24).** `sentCountsByStage()` in [lib/reporting/delivery-rollup.ts](../../lib/reporting/delivery-rollup.ts) sums `stage_delivery_rollup.sent` for every **closed** ET day in the window and counts **today** live from `stage_sends`. Both halves use the same definition (`status='sent'`, bucketed by `sent_at` in ET), which is why they add up exactly.

- **Why today stays live.** Total Sent is the number an operator watches mid-send to confirm a campaign is actually going out; a 10-minute rollup lag would read as a stall and get escalated. That is also why Total Sent carries **no "as of" label**, unlike Delivered %.
- **Why a day-grain rollup is exact here, not approximate.** `getStageMetricsInRange()` takes `from`/`to` as ET **date strings** and derives both bounds with `fromZonedTime` on midnight, so the window is ET-day aligned **by construction** and no caller can pass a partial day. If that ever changes, this read is wrong and the live count must come back.
- **`send_date` keeps the live count.** Under that basis there is no window at all — a cohort stage's metrics are everything it has ever produced — and the rollup's earliest cell is its backfill, not the beginning of time. The hybrid applies to the windowed basis only.
- **What it replaced.** One `count(*) … GROUP BY stage_id` over `stage_sends`, measured on prod 2026-09-24 at **1.6–1.7 GB and 10–12 s** on a 92-day window — the dominant cost of both `/api/keitaro/reports` and `/api/reports/performance`.

| window | rows compared | sends | live | hybrid | speedup |
|---|---|---|---|---|---|
| 1d (2026-09-23) | 28 | 45,735 | 86 ms | 50 ms | 1.7× |
| 7d | 207 | 314,165 | 283 ms | 109 ms | 2.6× |
| 14d | 474 | 751,837 | 2,525 ms | 110 ms | 23× |
| 92d | 2,085 | 5,149,799 | 10,401 ms | 204 ms | 51× |

Every row identical, 0 diffs, in one `REPEATABLE READ READ ONLY` snapshot — [scripts/verify-hybrid-sent.ts](../../scripts/verify-hybrid-sent.ts). The ET-midnight seam is pinned by [scripts/test-hybrid-sent-boundary.ts](../../scripts/test-hybrid-sent-boundary.ts) (preview, 6 bars), whose fixture **poisons the rollup cell for today** with 999,999 so that code which wrongly reads today from the rollup fails loudly instead of being plausibly off.
...
- ⭐ **Row order (2026-09-23, owner's rules — applies to Overview AND the By-X tabs).** A header click sorts **descending first**, ascending second, and never clears; then **Clickers descending** as a permanent secondary that never flips, whatever the primary column or direction; then a **stable key** (Overview: `campaign_id` then `stage_id`, because `groupBy=stage` makes campaign_id non-unique — By-X: the row's `key`). The key order lives once, in [lib/reporting/report-sort.ts](../../lib/reporting/report-sort.ts) (`makeOverviewComparator` / `makeDimensionComparator`), and the click cycle in [lib/ui/sort-cycle.ts](../../lib/ui/sort-cycle.ts) — opt-in via `DataTable`'s `sortCycle` prop, because that wrapper backs every registry list and they keep `asc → desc → clear`; the By-X tabs' `toggleSort` calls that same function (2026-09-23) instead of repeating the rule. **Bug fixed in the same change:** both comparators folded the tie-break into `cmp` *before* negating for a descending sort, so the tie-break reversed with the direction. Overview's sort runs **before the page slice**, so its secondary key has to be server-side — an ordering now pinned on the route's source, not just asserted in prose. A null sorts **last in both directions on every kind of column**, Overview's plain numeric columns included (they used to coerce with `?? 0`). 15 bars in [scripts/test-report-sort.ts](../../scripts/test-report-sort.ts) (pure, no DB). See [07-conventions.md](../07-conventions.md).
- ⭐ **The first column is frozen on Overview AND on every By-X tab (2026-09-23).** These tables are 25–30 columns wide, so the campaign name / dimension name would scroll off the left edge long before the numbers worth comparing arrive. The first cell is pinned with the shared bundle in [lib/ui/frozen-column.ts](../../lib/ui/frozen-column.ts) — **opt-in per table**, because [components/data-table.tsx](../../components/data-table.tsx) backs ~20 narrower registry lists that keep their default (`freezeFirstColumn` defaults to false; Overview is the only caller, and the By-X table applies the same constant to its own markup). The frozen cell paints an opaque background and re-applies its row's hover tint on top, so the scrolling columns pass BEHIND it instead of through it. Bars in [scripts/test-frozen-first-column.ts](../../scripts/test-frozen-first-column.ts); the behaviour itself is proven in a browser, not by those bars. See [07-conventions.md](../07-conventions.md).
