...
| **Vercel Cron** | scheduler → app | the 8 cron endpoints | `Authorization: Bearer CRON_SECRET` | staggered `*/15`/`*/5` pollers + `0 * * * *` (hourly telegram-report) + `0 5,20 * * *` (twice-daily offer-group-report refresh) |
| **Telnyx Number Lookup** _(in build)_ | app → service | carrier/line-type enrichment (`lib/telnyx/`) | `Authorization: Bearer TELNYX_API_KEY` | lookup: `GET https://api.telnyx.com/v2/number_lookup/{+E164}?type=carrier` → `{data:{carrier:{name,type},portability:{line_type,lrn,ocn,spid,ported_status,ported_date}}}`; balance: `GET /v2/balance` → `{data:{available_credit,balance,currency}}` (string fields) |
