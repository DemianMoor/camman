...
### Task 7: Write path — POST stores encrypted + last4 + label

**Files:** Modify `app/api/providers/[providerId]/credentials/route.ts` (POST + GET), `lib/validators/providers.ts`.

**Interfaces:**
- Consumes `encryptSecret`, `maskApiKey`. Produces rows with `api_key_encrypted` set, `api_key` NULL, `api_key_last4` + `label` set.

- [ ] **Step 1 (validator):** Extend `providerCredentialSetSchema` with `label: z.string().trim().min(1, "Label is required").max(120)`. Keep `brand_id` optional, `api_key` required.
- [ ] **Step 2 (POST):** In the transaction, compute `const { last4 } = maskApiKey(api_key); const enc = encryptSecret(api_key);`. On insert set `{ org_id, provider_id, brand_id, label, api_key_encrypted: enc, api_key_last4: last4 }` (do NOT set `api_key`). On rotate (existing row) set `{ api_key_encrypted: enc, api_key_last4: last4, api_key: null, updated_at: new Date() }`. Gate stays a mutation → change `can(role,"providers.update")` → `can(role,"provider_credentials.manage")`.

Note: multi-account means the "upsert by (provider, brand)" logic is gone — creation always inserts a new row; rotate targets a specific `credentialId` (handled by the PATCH route in Task 10). The POST becomes create-only. Keep the numberless-stage guardrail check here too (see Task 10 Step 3) since POST is where a 2nd account is born.

- [ ] **Step 3 (GET):** Change the gate to `can(role,"provider_credentials.view")`. Select `id, brand_id, label, api_key_last4, updated_at` + a linked-numbers subquery: `(SELECT count(*) FROM provider_phones ph WHERE ph.credential_id = pc.id) AS linked_numbers`. Return `last4`/`masked` from `api_key_last4` (fallback: if `api_key_last4` is NULL — pre-backfill row — derive from plaintext via `maskApiKey`, still never returning the key). **No decryption in GET.**
...
### Task 10: Sequencing guardrail on account creation

**Files:** Modify `app/api/providers/[providerId]/credentials/route.ts` (POST).

**Interfaces:** Consumes nothing new. Produces a 409 when a 2nd+ account would make numberless stages ambiguous.
...
- [ ] **Step 6: Gate the mutate buttons** (Add/Rotate/Edit/Delete/Send test/STOP callback) behind a `can(role,'provider_credentials.manage')` prop passed from the server page; view-only (manager) sees the list but not the actions. Keep the list visible at `provider_credentials.view`.
