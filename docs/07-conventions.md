...
- **Threshold RESOLUTION has one implementation too.** `createThresholdTempTables` ([lib/engagement/thresholds-sql.ts](../lib/engagement/thresholds-sql.ts)) builds the org row and the strictest-across-active-groups aggregate for both the job and the settings preview. A preview of UNSAVED values injects them there (`proposedOrg` / `proposedGroup`) rather than writing and rolling back, and it reads the STORED facts — a preview must never recount `stage_sends` (that is the two-minute backfill pass, not a button).
...
- **An empty audience chip set matches NOBODY (2026-09-25).** A lifecycle campaign picks its audience with the five status chips, and of the two readings of "none selected", the one that silently messages the entire contact base is the one that must not be the default. Enforced in three places -- the form, `audienceFiltersSchema`, and the SQL predicate itself -- because the first two are the ones a future refactor removes. `suppressed` is never an offerable chip: it is excluded as a LAYER, so the reason is reported instead of a status simply being absent from a list.
...
- **Spread a shared type; never list the keys.** `LifecycleExclusionCounts` ([lib/sends/eligibility.ts](../lib/sends/eligibility.ts)) is the one declaration. Every shape spreads it, and the two client copies reference the server type outright, so an unhandled new bucket is a compile error rather than a silent zero.
- **DERIVE the key set from the priority order, by difference.** `LIFECYCLE_EXCLUSION_KEYS` is `EXCLUSION_PRIORITY` minus the content-dedup keys. A hand-written list would be the fifth copy the arrangement exists to prevent.
- ⭐ **A runtime bar cannot catch a copy that agrees today** -- and agreeing today is exactly how the original four drifted apart. [scripts/test-exclusion-bucket-drift.ts](../scripts/test-exclusion-bucket-drift.ts) therefore pairs the runtime key-set check with a **source scan** asserting no file outside the canonical one declares the buckets itself. The source half found a real copy on its first run that the runtime half could not see.
- **Report the layers per layer, not folded together.** Before this, the lifecycle layers were counted inside `content_dedup`, so a suppressed lead was reported as a creative repeat. `computeStageReconciliation` now splits the bucket per layer, exclusively and in `EXCLUSION_PRIORITY` order -- "excluded 4,102" becomes "3,900 suppressed, 202 freeze not due", which is the difference between a number an operator can act on and one they can only stare at.

## A warning shown on two pages must be decided in one place (2026-09-25)

The Excl-timing warning appears from the campaign LIST page and the campaign DETAIL page, whose data arrives by completely different routes: the detail page holds the campaign and its stages in client state, the list page has to prefetch them. The failure mode worth guarding is not "the warning is wrong" -- it is "the warning is right on one page and absent on the other", because an absent warning reads as *nothing to worry about*.

- **One pure function, called by both.** `shouldWarnExclTiming` / `exclTimingInput` ([lib/campaigns/excl-timing-warning.ts](../lib/campaigns/excl-timing-warning.ts)). Neither page decides anything itself, and `now` is injected rather than read, so the decision is testable.
- ⭐ **Test that both mount sites receive the same INPUTS, not that both warn.** Two pages that agree today by coincidence are the thing that drifts. [scripts/test-excl-timing-warning.ts](../scripts/test-excl-timing-warning.ts) H10 feeds one campaign through both paths and asserts an identical argument object, which makes an identical verdict unavoidable rather than lucky.
- **Prefetch for the dialog; do not widen the list route.** `campaigns/list` serves every row of a 670-campaign list on every page load. Data that only 8 campaigns use, only at the moment a dialog opens, is fetched when the dialog opens (owner decision, 2026-09-25). `GET /api/campaigns/[campaignId]` gained `earliest_scheduled_at` as a `min()` on the stage aggregate it already ran -- one campaign, no extra round trip.
- **Hold confirm while the prefetch is in flight.** `undefined` (still fetching) and `null` (nothing to warn about) are deliberately different values. A dialog that can be confirmed a moment before its warning appears is worse than one that waits.
- **Capture `now` in the open handler, not during render.** `Date.now()` in JSX is impure (`react-hooks/purity`) AND re-runs every render, quietly moving the 24 h boundary under an open dialog.
...
- ⭐ **A dynamic `import()` of the db client counts as reaching a database (2026-09-22).** Before this change `DB_REACH` saw only a *static* `from "…/db/client"`. A script that loaded `.env.local` with `dotenv` and reached the client only through `await import("@/db/client")` matched no needle and was never enrolled, so a bare run wrote to production. That is the shape a `server-only` stub forces, because the stub has to be in place before any app module loads. Four scripts had it: [`test-lookup-worker.ts`](../scripts/test-lookup-worker.ts) (it rewrites the global `lookup_settings` row), [`test-lookup-uploads.ts`](../scripts/test-lookup-uploads.ts), [`test-eligible-gate.ts`](../scripts/test-eligible-gate.ts) and [`backfill-carrier-v2.ts`](../scripts/backfill-carrier-v2.ts). The fix:
  - **Two named needles**, one per spelling the tree uses: `import(@/db/client)` and `import(../db/client)`. Each has a hand-written sample and a hand-written **near miss** (`import("@/db/schema")`, which opens no connection). A narrowed needle turns its sample bar red; a widened one turns its near-miss bar red.
  - **The three tests are guarded**: `_env-preload` goes first, then `_require-preview-db`. A `server-only` stub stays, but it comes *after* line 2, and the dynamic imports stay inside `main()`.
  - **The backfill goes in `EXCLUSIONS`.** It is a deliberate production tool: dry-run by default, and `--apply` snapshots to a rollback table first.
  - **Nothing else was hiding.** After the change the detector newly classifies exactly those four as write-capable (183 → 187).
  - **Nine more scripts reach the client only through `import()` but have no write token of their own**, so they stay unenrolled. That is the via-library limit stated in the bar's header, and it is unchanged.
...
- **UPDATED 2026-09-18 — the hourly tab's sales/revenue INSTANT changed** from `stage_sends.converted_at` (Keitaro's latest re-post time, which MOVES — a re-post carried revenue into a later hour, and out of the range once it crossed ET midnight) to `conversion_events.occurred_at` (the conversion's own time, which never moves). This is the only Task 4 change that can move a live number, and it is recorded as correction class **D** in the Phase 3 plan so Task 7's before/after table classifies it instead of reading it as a regression.
...
Guards:
- [`scripts/test-segment-purchase-rules-db.ts`](../scripts/test-segment-purchase-rules-db.ts) — preview DB only. Runs the REAL `buildSegmentAudienceClause` for all three `made_purchase*` rules and `computeAudiencePools`, against one-sided fixtures (ledger-only / legacy-only / rejected / registration / other brand / other offer / other org), plus the `is_not` complement. Contacts, sends and ledger rows live in a transaction that always rolls back; only the segment + registry rows the builder must read through the module `db` pool are committed, and they are hard-deleted and residue-checked. **This is the script to extend for rule semantics** — `test-purchase-rules.ts` drives the HTTP API and needs an anon key the preview project does not expose locally.
...
- **⭐ A watchdog must reproduce the predicate of the thing it watches, or it reports the system's normal behaviour as a fault.** The stall detector listed every hold it knew about — campaign pause, provider pause, org switch, org emergency stop, quiet hours — but not the drain's **pacing ceilings**, so a provider at its configured `max_sends_per_24h` read as a stall. On 2026-08-27 Text Request sat at 30,038 against a 30,000/24h ceiling and the detector called it 3,976 undraining messages, advising "check the send-scheduled cron / provider health" about a queue obeying its own configuration. The fix is not a new heuristic but the drain's OWN test, reused: `countSentSince(org, provider, window) >= resolve*Cap(...)` is literally what makes the drain `break` with `stopReason = "rate_24h"`. **When you add a stop condition to a worker, add it to that worker's watchdog in the same change** — the detector's whole value is that a hold it cannot explain means something is wrong, and every unmodelled hold erodes that. See `hold` / `trulyStalled()` in [lib/sends/stall-detector.ts](../lib/sends/stall-detector.ts).
- **⭐ A watchdog's grace period must start when the work became ELIGIBLE, not when its inputs were prepared.** Same family as the rule above, and the half that was still missing: the stall detector modelled the drain's *stop* conditions but anchored its "give it time to get going" grace to `campaign_stages.materialized_at`. That was sound while materialization happened at send time; it stopped being sound once materialization became a pre-pass running **hours ahead** (205 of 207 stages in a 7-day sample led their `scheduled_at` by >30 min). A stage then arrived at its due moment with the entire grace already spent, so it was alarm-eligible the instant it was permitted to send — and the only remaining suppressor, "has sent something recently", is precisely the thing a not-yet-started stage cannot have. On 2026-08-29 that fired `2934 pending, never sent` at a queue that was working: four stages came due at `14:00:00`, the drain serves them round-robin, and the hourly cron landed in the **29 seconds** between the third stage's first send and the fourth's. **Ask what instant makes the work legal to start — due time, release, dependency satisfied — and measure patience from the latest of those**, not from whatever timestamp happens to be handy. A pre-computation step moving earlier is a silent, unrelated-looking change that expires this kind of grace. Now `GREATEST(materialized_at, COALESCE(scheduled_at, …), COALESCE(sent_at, …))` in [lib/sends/stall-detector.ts](../lib/sends/stall-detector.ts).
- **⭐ An exclusion that is correct for one monitor is a blind spot unless some other monitor owns it.** `findStalledStages` excludes `schedule_missed_at IS NOT NULL` and is right to — a stood-down stage is not failing to drain. But nothing else looked at stood-down stages either, so the exclusion silently created a category no alert covered: stage 3272 held **2,000 undispatched messages for 35 hours** and would have held them indefinitely. Whenever a check narrows its scope for a good reason, name where the excluded case goes; if the answer is "nowhere", that is the bug. Now owned by [lib/sends/missed-stages.ts](../lib/sends/missed-stages.ts).
- **⭐ A per-entity alert must clear as deliberately as it fires.** `notifyOnTransition` latches on `alert_state`, which is what stops an hourly cron repeating the same alert forever — but a latch nobody resets fires once and is then permanently deaf for that entity. Callers that evaluate a SET (rather than one entity with an explicit boolean, as `reportExhaustion` does) cannot clear by omission: the recovered entity simply stops appearing. `reportMissedStages` therefore re-reads `alert_state` for its own key prefix and clears every `firing` key absent from the current set. The guard asserts the full lifecycle — fires once, silent on the second tick, cleared on recovery, **audible again on recurrence** — because only that last assertion distinguishes a working latch from a broken one.
...
- **"Completed stage" is `sent_at` + no open rows — NEVER `campaign_stages.status` (migration 0174).** There is exactly ONE definition, `stageCompleteExpr()` in [lib/sends/stage-complete.ts](../lib/sends/stage-complete.ts): `sent_at IS NOT NULL AND NOT EXISTS (stage_sends WHERE status IN ('pending','sending'))`. It is shared by the campaign-level behavioural split's source set AND the P4 parent-complete gate (`getParentState`) so the two cannot drift. `status` is the OPERATOR'S MANUAL RECORD of campaign results and disagrees with the pipeline: measured on production 2026-08-27, of 1,231 tracked stages **1,183 have `sent_at` but only 957 carry a status in (`success`,`sent`)** — filtering the source set on `status` would silently drop 227 stages that really sent. `failed`/`skipped_*` rows are terminal and do NOT block completeness; `sending` does.
...
- **A lane that finishes materializing must also SETTLE its group — from every path, which is why the settle lives in `kickoffStageSend` (0174).** Phase A settles too, but Phase A only ever selects lanes with `materialized_at IS NULL`, so a lane finished by the **manual Prepare button** was never followed by a settle from anywhere. Phase B gates on `state='materialized'`, so those already-prepared messages would NEVER release — a **silent non-send**, the worst outcome this design can produce. Found live 2026-08-28 on campaigns 1032 and 1033: both groups fully materialized, both stuck in `materializing`, both scheduled to fire that afternoon, 366 prepared messages that would not have gone out. `settleCompletedSplitGroups()` on the send-preflight cron is the self-healing backstop (it runs BEFORE the stuck sweep, so a merely-un-settled group is healed rather than alerted on). Same shape as the resolve: whatever can finish a lane must be able to finish its group.
...
- **A copy MUST carry `landing_page_id` — all four copy paths, not just one.** Fixed 2026-08-28. Migration 0150 introduced `campaign_stages.landing_page_id` as the destination ("a copy carries the landing PAGE, not a frozen URL, so it is rebuilt at mint time from ITS campaign's brand"), but only the **stage-duplicate** path was updated. Behavioural split, A/B split and campaign-duplicate all dropped it, so every stage they produced had NO destination and `kickoffStageSend` refused it with `no_destination` — the operator had to re-pick the landing page by hand on each lane/variant. Observed live on campaign 1062. Legacy `full_url` stages were unaffected, which is why it stayed hidden. When adding any destination-bearing column, grep all four copy paths.
...
- **Long single-select lists get a type-to-search dropdown, not a plain `<Select>`.** `<SearchableSelect>` ([components/searchable-select.tsx](../components/searchable-select.tsx)) is the single-select sibling of `<MultiSelectPicker>` — same Radix Popover + filter-input shape, commits one value and closes on pick, with ↑/↓/Enter keyboard nav. Its trigger deliberately mirrors `<SelectTrigger>`'s styling so swapping one in doesn't shift the surrounding layout. Rule of thumb: **≤10 options stay a plain `<Select>`** (operator, AND/OR, lookback period); more than that, make it searchable. The Today's-Sends number filter sits ABOVE that bar: 3-7 numbers were in play on any of the 21 days sampled, but the operator confirmed the real range exceeds 10, so it is an ordinary application of the rule, not an exception. **A measured range is not an operating range** - sampling recent days bounds what HAS happened, never what the tool must hold.
  - **`searchText` (optional) is matched alongside `label` and never rendered.** Use it when the displayed label doesn't literally contain a form the user will type. The number filter sets it to the raw E.164 digits so `+1 844 621 0404` is found by typing `8446210404`; without it the search box is useless for the exact thing it filters on. Extend this component rather than adding a fourth popover-search one.
...
- **`campaign_stages.delivered_count` and `sms_count` are written only by CSV result imports and manual results.** API sends write neither. On 2026-09-14 both were 0 on all 915 creative stages of the last 30 days, which had sent 1,706,771 messages — so the creatives CTR (`clean clicks ÷ delivered`) rendered "—" on every creative. A send is `stage_sends.status = 'sent'` on a tracked stage and `sms_count` on a manual one (the rule in [lib/reporting/grading.ts](../lib/reporting/grading.ts) and [lib/stages/total-cost.ts](../lib/stages/total-cost.ts)). **Before dividing by a stage counter, check who writes it.**
- **A per-creative send count is a full `stage_sends` pass**: 11.5 s all time, 12.2 s for 30 days through `stage_sends_org_sent_at_idx` (it doesn't cover `status`/`stage_id`), 4.3 s as an index-only count that can't see status. So CTR comes from an hourly snapshot ([lib/creatives/ctr-rollup.ts](../lib/creatives/ctr-rollup.ts), `operator_rollups` key `creative_ctr`), never from a list read.
- **The numerator and the denominator come from the same statement.** A live clicker count over an hourly send count climbs between refreshes and snaps back — the same drift the EPC cache avoids by riding the Keitaro poll.
- **CTR's all-time sends exceed the Reports "sent" total, by definition.** The report cohort ([lib/reporting/stage-funnel.ts](../lib/reporting/stage-funnel.ts)) requires `campaign_stages.sent_at` stamped and `archived_at` null. CTR also counts manual stages whose `sent_at` was never stamped (112,348 `sms_count` + 4 tracked sends on 2026-09-14 — exactly the gap). `scripts/verify-creative-ctr.ts` rebuilds the report's stage set and asserts the rest is only sends after the stored run.
...
**Fixed 2026-09-17 (Phase 3 Task 3, bug 2).** `keitaro_stage_results`' conversion columns (`checkouts`/`sales`/`revenue`/`payout_at_conversion`) are no longer written by the aggregate poll's `conversions/log` fold — that fold (`applyConversionRowToAggregate`) is deleted. They are now a **projection of the ledger**, re-derived by [`lib/keitaro/stage-day-conversions.ts`](../lib/keitaro/stage-day-conversions.ts)'s `syncStageDayConversions`, dated by `occurred_at`: each (stage, ET day) the ledger explains is INSERT/UPDATEd from a fresh `GROUP BY`, and any existing row the ledger no longer explains for that day is zeroed (conversion columns only — click columns on the same row are untouched). Re-deriving instead of folding makes the double count structurally impossible: there is no "old day's row" to go stale, because every run recomputes every day in scope from scratch. `discoverChangedLedgerStages` (a resumable `conversion_events.updated_at` window, migration 0182's `conversion_events_updated_at_idx`) is what lets the `*/5` route (`app/api/keitaro/poll/route.ts`) find a re-posted OLD conversion whose stage-day sits outside the current click window. One-shot repair for stage-days frozen before this shipped: `scripts/resync-stage-day-conversions.ts` (dry-run by default).

⭐ **A re-derivation must never outrun its source's coverage.** The first version of this projection zeroed every in-scope stage-day the ledger did not explain, with no date floor — correct against a complete ledger, catastrophic against a real one: the live ingest window is 7 ET days, so on prod (where the Phase 1 backfill had not run) the first tick would have zeroed MONTHS of real sales and revenue for every stage with a click in the last 3 days, silently, on the table every sales report reads. Two bounds now, both in SQL, both in `syncStageDayConversions`:

- **Refuse when the source is empty.** No stage-attributed `conversion_events` row anywhere ⇒ write nothing, return `refused: "empty_ledger"`, page `conversion_events:projection_failed`. An empty source is not evidence that the target is wrong.
- **Floor the destruction at the source's own earliest row, PER GROUPING KEY.** A stage-day is only zeroable when that stage's earliest ledger conversion is on or before it. Per stage, not one global floor — a global floor still zeroes history for every stage the backfill has not reached. A stage with no ledger rows at all is therefore never touched.
- **Refuse when the source is BEHIND the target's history** (added in the second review wave, same day). ⭐ **An overwrite needs the same coverage bound as a delete.** The two bounds above only guard the ZEROING; the upsert's `DO UPDATE SET checkouts/sales/revenue = EXCLUDED` was left unbounded, so a day the ledger covered only PARTIALLY — an interrupted backfill, or one old re-posted conversion dragging a stage's floor back months, which is enough to put every day since inside "coverage" — was silently REDUCED to the partial sum. Same damage as the zeroing bug, through the other statement. The guard: once per run, before any write, compare the earliest conversion day the target still reports (`min(stat_date)` over `keitaro_stage_results` rows with a non-zero conversion column) against the source's earliest stage-attributed day. Source later ⇒ refuse everything, `refused: "ledger_behind_history"`, page both dates. Global, not per scope: a half-loaded source is a whole-table condition, and a scoped run would otherwise reduce whatever stages it happens to name. Written as a `stat_date < ledger_floor` probe so the healthy case is an index range that matches no rows, not a full scan of the biggest table on the tick.

The generalisation: **any job that derives a table from another and deletes _or overwrites_ what it cannot explain needs a coverage bound, not just a correctness proof.** "The ledger doesn't explain this row" and "the ledger hasn't been loaded that far back" look identical to the SQL; only a floor tells them apart. And the bound has to cover every statement that narrows a number — the second review wave found the first fix had bounded the DELETE-shaped one and left the UPDATE-shaped one open. What no floor can catch is a GAP inside the covered range; that one belongs to the loader's own verify (`scripts/verify-conversion-events.ts` against a fresh Keitaro pull), and it is worth writing down which failure each layer owns. The ordering is part of the fix, not a nicety — the backfill runs on prod BEFORE the projecting code is live (Task 8's precondition list); the floors and the refusals are the net if that order slips.
