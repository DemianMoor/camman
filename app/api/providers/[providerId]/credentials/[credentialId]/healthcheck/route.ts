...
import { textrequestHealthcheck } from "@/lib/sends/providers/textrequest";
...
// GET — non-sending Text Request connection check for a stored credential.
// Resolves the account's key server-side and calls Text Request's
// GET /dashboards to confirm the key authenticates and to surface the account's
// dashboards (each dashboard is 1:1 with a sending number; the operator binds a
// dashboard id to a provider_phone). Read-only: NO SMS is sent, no spend — so it
// is NOT gated by SEND_ENABLED (unlike the test-send route). Admin+
// (provider_credentials.manage) because it resolves and transmits the plaintext
// key to an external service, matching the register-callback route's bar. The
// key is never returned.
//
// Text Request-only: the other providers authenticate differently and have no
// equivalent dashboards endpoint, so a non-txr credential is rejected rather
// than misrouted (a txh key sent to Text Request would just fail auth).
export async function GET(
  ...
  if (row[0].providerKey !== "txr") {
    return apiError(
      400,
      "Connection check is only available for Text Request accounts",
      API_ERROR_CODES.VALIDATION,
      { reason: "provider_not_textrequest" },
    );
  }
  ...
  const result = await textrequestHealthcheck(apiKey);
