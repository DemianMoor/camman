...
- For **single**-select with many options (>10), use `<SearchableSelect>` from `components/searchable-select.tsx` — the single-select sibling of the above (popover + type-to-filter input + ↑/↓/Enter nav; commits one value and closes on pick). Its trigger mirrors `<SelectTrigger>` styling, so it can replace a plain `<Select>` in place. **≤10 options stay a plain `<Select>`** — a filter box over a handful of items is friction. `<OfferPicker>`/`<SegmentPicker>` are the specialized variants; use them only when the pin + recently-used ordering is wanted. Don't add a fourth popover-search component — extend one of these.
...
**Audience source composition (segment ∩ group).** Segments **AND** together (a contact must be in EVERY selected segment), contact groups OR together (ANY selected group), and the two dimensions **INTERSECT** when both are populated — a contact must be in every selected segment **AND** a selected group. When only one dimension is filled, that side is used alone (the empty dimension is ignored, not "match nothing"). The shared `buildAudienceSourceClause` in [lib/audience-snapshot.ts](lib/audience-snapshot.ts) builds this for the snapshot and the draft stage count; `previewAudience` applies the same rule via a `membership_ok` flag so its breakdown can still report each side's pool (`from_segments` / `from_groups`) while `total_matching` / `overlap` are the intersected audience. (Earlier behavior UNION'd both dimensions; changed 2026-06-10.)

**Segments INTERSECT across a campaign (changed 2026-08-17; previously UNION).** Each additional include-segment can only NARROW the audience. The old UNION detonated on "filter-shaped" segments — one whose only rule is an `is_not` (e.g. `in_use_in_campaign_last_period is_not 1w`) matches nearly the whole org, so OR-ing it in grew a 44,480 audience to 507,870. `buildAudienceSourceClause` is shared with `snapshotAudience`, so activation would have FROZEN AND SENT to that inflated pool — a live-send hazard, not a display bug. To OR two audiences together, model it as two `or`-combined rules inside ONE segment; UNION lives inside a segment, not across them. **Two independent implementations must stay in lockstep:** `buildAudienceSourceClause` (an INTERSECT chain) and `previewAudience` (a `segments_matched = <n>` test over per-branch ordinals — it can't use `bool_or` because it must distinguish "in some segment" from "in all of them"). If they drift, the preview stops predicting what activation freezes. Guard: `scripts/test-segment-intersect-and-optout.ts` asserts both paths land on the same count and that adding a segment never grows the audience. **Perf:** when both dimensions are present, the group set is passed to `buildSegmentAudienceClause(…, restrictUniverse)` as the `is_not` universe so a near-universal negated rule doesn't scan all contacts before the intersection; status flags are hash-joined (`flagSetCtes`/`flagJoins`) not correlated-`EXISTS`. Don't regress these (it's the difference between ~9s and ~0.4s at scale). A side effect: `from_segments` then reflects the segment evaluated within the group (= the intersection), not the full segment side. **Snapshot path (`snapshotAudience`): the candidate set is materialized into a `ON COMMIT DROP` temp table and `ANALYZE`d before the flag joins.** This is load-bearing, not optional cleanup: the source is built from `UNION`/`INTERSECT`/`EXCEPT` set ops whose cardinality Postgres estimates at ~200 rows, so at scale (150K+ candidates) the planner picks nested-loop anti-joins for the opt-out/in-use exclusions and the activation hits `statement timeout (57014)`. The temp table gives real row stats → hash anti-joins (>180s → ~8.5s). Because temp tables need a transaction, `snapshotAudience` must be called with the activation `tx`, and the two routes that snapshot set `maxDuration = 60`. **The prior-offer exclusion (`exclude_prior_offer_contacts`) runs as a SEPARATE `DELETE … WHERE EXISTS (offer_exposures …)` after `audience_qualified` is materialized and `ANALYZE`d — do not fold it back into the qualifier.** Inline it becomes an anti-join whose outer side estimates `rows=1`, against an `offer_id` that is often missing from the `offer_exposures.offer_id` MCV list (also estimated at 1 row): the planner picks a nested loop and re-scans `offer_exposures` once per candidate (measured 115.7M heap fetches, 84s ⇒ the 60s route limit killed campaign creation). Split out it hash-anti-joins: 67.5s → 4.2s, identical rows.
...
- **`status = 'sent'` on `stage_sends` is the single shared definition of "was messaged."** The reports rollup, the send circuit breakers, and the `sent_from_provider_phone` rule (`{provider_id, phone_ids[]}`, `is`/`is_not`, no time window; migration 0129) all key off the same status value, backed by partial index `stage_sends_org_provider_phone_sent_idx`.
- **A set-shaped rule value (array/object, not a scalar FK id) must be registered in FOUR places, not two: `RULE_TYPES`, `validateValueByShape`, `isRuleComplete`, and `verifyValueOwnership`.** The last two both fall through to a `typeof value === "number"` test by default and silently mishandle a set-shaped value unless explicitly branched. Missing them is exactly how `phone_type`/`carrier` (migration 0098) shipped uncreatable — fixed alongside `sent_from_provider_phone` (migration 0129), the first rule type to exercise this path successfully. See [docs/07-conventions.md](docs/07-conventions.md).
- **Counts:**
  - `segment_stats.total_count` (per-row trigger) is the manual-membership count — unaffected by rules.
  - `segment_stats.rule_filtered_count` (computed on demand by `/api/segments/[id]/refresh-stats`) is the **SENDABLE** audience count: the full UNION'd audience (manual ∪ rule_matches) MINUS opt-outs. Null when no active rules exist or when the eval timed out. The column name is historical — it really means `sendable_audience_count`, not a narrowed subset.
  - **Opt-outs are excluded from every segment-page audience read (changed 2026-08-17)** — header tile, Audience tab (list AND counts), rules preview, and the contacts CSV export — via `excludeOptOutsFromAudience()` in [lib/segment-rules-eval.ts](lib/segment-rules-eval.ts). Before this the segment page advertised an audience no campaign could draw: 54,356 on the page vs 44,480 as a campaign's "From segments", the gap being exactly its 9,876 opt-outs. **Do NOT fold this into `buildSegmentAudienceClause`** — the campaign path intentionally keeps opt-outs in its source so `previewAudience` can report `excluded_for_optout` before `qualifies` strips them, and the snapshot anti-joins them once against an ANALYZE'd temp table; duplicating that anti-join into every segment branch would tax the perf-critical activation path for nothing. The export excludes them for compliance, not just arithmetic — that CSV goes straight into a provider tool.
  - `segment_stats.opt_out_count` / `opt_in_count` / `clicker_count` are computed over the **full audience** (pre-opt-out-suppression) when the segment has active rules, so the page reconciles: `rule_filtered_count + opt_out_count = full audience`. They previously counted manual `segment_contacts` membership only — empty for most rule-based segments, so a segment named "Clickers excl Buyers" displayed `Clickers 0` and `Opt-Outs 0` while 9,876 of its audience were suppressed. That silence is why the missing contacts were invisible. No-rule segments keep the manual figures, and a timed-out audience query falls back to them (the columns are NOT NULL — never persist zeros on timeout).
...
- **Campaign audience snapshots respect a segment's internal UNION.** [lib/audience-snapshot.ts](lib/audience-snapshot.ts) calls `buildSegmentAudienceClause(segmentId, orgId)` per segment and **INTERSECTs** across segments (see §10b — changed 2026-08-17; it used to UNION). Existing frozen pools (`campaign_audience_pool` rows) are NOT recomputed when rules change after a campaign moves past draft; this is by design.
- **UI conventions:**
