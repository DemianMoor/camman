...
export default function ClickersPage() {
  ...
  useEffect(() => {
    (async () => {
      const r = await offersApi.execute("/api/offers/list?pageSize=100");
  ...
  useEffect(() => {
    ...
    if (filters.offer_id !== null)
      params.set("offer_id", String(filters.offer_id));

    (async () => {
      const result = await listApi.execute(
        `/api/clickers/list?${params.toString()}`,
      );
      if (cancelled) return;
      if (result.ok) {
  ...
  const canUpload = can("clickers.upload");
  const canDelete = can("clickers.delete");
  ...
  function handleUploadSuccess(summary: UploadResultSummary) {
    toast.success(
      `Added ${summary.inserted.toLocaleString()} clicker${summary.inserted === 1 ? "" : "s"}`,
    );
    refetch();
  }
  ...
  async function handleBulkDelete() {
    ...
    const result = await bulkDeleteApi.execute("/api/clickers/bulk-delete", {
      method: "POST",
    ...
    toast.success(`Deleted ${result.data.deleted_clickers} clickers`);
    setBulkDeleteConfirm(false);
    setSelectedRows(new Set());
    refetch();
  }

  const columns = useMemo<ColumnDef<Clicker>[]>(
    () => [
      {
        id: "select",
        header: () => null,
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Clickers</h1>
          <p className="text-sm text-muted-foreground">
            Contacts who clicked a tracked link in a campaign.
          </p>
        </div>
        {canUpload ? (
          <Button
          ...
          >
            <Plus className="size-4" aria-hidden /> Add Clickers
          </Button>
        ) : null}
      </header>
      ...
      {fetchError ? (
        <div className="rounded-md border border-destructive/40 bg-destructive/5 p-4 text-sm">
          <p className="text-destructive">
            Couldn&apos;t load clickers: {fetchError}
          </p>
        ...
        <div className={cn("flex flex-col items-center justify-center gap-4 rounded-md border border-dashed py-16 text-center")}>
          <MousePointer className="size-12 text-muted-foreground/40" aria-hidden />
          <div className="space-y-1">
            <p className="text-sm font-medium">No clickers yet</p>
            <p className="text-sm text-muted-foreground">
              Add contacts who clicked tracked links.
            </p>
          </div>
          {canUpload ? (
        ...
        <div className="flex flex-col items-center justify-center gap-4 rounded-md border border-dashed py-16 text-center">
          <p className="text-sm text-muted-foreground">
            No clickers match your filters.
          </p>
          <Button
            variant="outline"
            size="sm"
                ...
                {offers.map((o) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setUploadOfferId(active ? null : o.id)
                      }
                      className={cn(
                        "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs",
                        active
      ...
      <AlertDialog open={bulkDeleteConfirm} onOpenChange={setBulkDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete {selectedRows.size} clicker{selectedRows.size === 1 ? "" : "s"}?
            </AlertDialogTitle>
            <AlertDialogDescription>
