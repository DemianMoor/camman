...
- **Campaign/stage attribution (migration 0075).** TextHub's inbox carries no campaign reference, so each STOP is reverse-matched to sends by phone + recency: the poller credits **every stage that sent to that number within a 72h trailing window** (`OPT_OUT_ATTRIBUTION_WINDOW_HOURS`, anchored on the parsed `provider_received_at`), one `opt_out_attributions` row per stage + a `campaign_stages.inbound_opt_out_count` bump. All inside the existing per-message transaction (idempotent via the `texthub_inbound_events` claim). The org-wide `opt_outs` row is unchanged — attribution is additive, never a suppression gate. No window match (CSV-only numbers, non-API providers, pre-pipeline sends) ⇒ org-wide opt-out only, counted `unattributed`. Backfill: [`scripts/backfill-optout-attributions.ts`](../../scripts/backfill-optout-attributions.ts) (anchors on `opt_outs.created_at`, our reliable UTC ingest clock — independent of the parse below). See [poll-opt-outs.ts](../../lib/sends/poll-opt-outs.ts).
- **TextHub `received_at` is UTC−6, not UTC (fixed 2026-06-19).** TextHub stamps inbound messages in a fixed UTC−6 wall clock with no zone suffix; `parseProviderReceivedAt` now shifts +6h to true UTC (`TEXTHUB_RECEIVED_AT_UTC_OFFSET_HOURS = -6`). Previously parsed as UTC, which put the anchor 6h in the past so a campaign's **own** STOP replies tripped the upper bound (`sent_at <= anchor + 5min`) and were dropped as "send-after-stop" — the stage's opt-out counter read 0 while ~100 recipients had actually replied STOP. Empirically confirmed: across 132 messages our ingest clock ran a rock-solid ~6h ahead of the stamped value. Suppression was never affected (the org-wide `opt_outs` row is still written); only stage-level attribution counters under-counted. **DST caveat:** the −6 offset was measured in June (DST); whether TextHub observes DST (i.e. fixed −6 vs `America/Denver`-style) is unconfirmed and may drift 1h across a DST boundary — revisit if winter counts look off.
