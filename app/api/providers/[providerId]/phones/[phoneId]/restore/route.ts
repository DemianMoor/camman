...
export async function POST(
  ...
  if (!can(role, "provider_phones.restore")) {
  ...
  // Restore always returns the phone to 'active', regardless of its
  // pre-archive status.
  const updated = await db
    ...
    .where(
      and(
        ...
        eq(provider_phones.status, "archived"),
      ),
    )
    .returning();
