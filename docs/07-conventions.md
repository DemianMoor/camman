...
- **Refuse when the source is empty.** No stage-attributed `conversion_events` row anywhere ⇒ write nothing, return `refused: "empty_ledger"`. An empty source is not evidence that the target is wrong.
