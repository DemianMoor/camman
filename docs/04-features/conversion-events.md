...
- **⚠️ Rule F's invariant is weakened until Task 6.** The rescue is now mapped-purchase-or-revenue and not rejected, while the EPC numerator (the stage-day projection) is still type-based, so a rejected or unmapped conversion could contribute revenue while its recipient sits outside `counted_clickers`. 0 rejected and 0 unmapped rows today; Task 6 closes it — see [epc-denominator.md](epc-denominator.md) §2.
- **The guard now carries six fixtures**, adding the three classes the first version could not see: a recipient with TWO ledger conversions (both must count — the +$715 class), an UNMAPPED row (the only class that SHRINKS a number: the legacy columns read it as a $55 sale), and a $0 registration (not rescued, not a purchase, rendered with no money). It also executes the real `refreshCountedClickers()` inside the rolled-back transaction, so Rule F is proven by the rows it actually writes. Proven RED against the pre-switch definitions (39 of 62 checks fail, including every new fixture's), restored byte-identically and `cmp`-verified.
