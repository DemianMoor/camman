...
async function main() {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      console.log(
        "Case 1: concurrent invocations on ONE phone — exactly one drains",
      );
      {
        ...
        const pA = runTick(f.orgId, fake).then((r) => {
          release();
          return r;
        });
        const pB = runTick(f.orgId, fake).then((r) => {
          release();
          return r;
        });
        ...
        check(
          "exactly ONE invocation drained the phone",
          winners.length === 1,
          `a=${JSON.stringify(a)} b=${JSON.stringify(b)}`,
        );
        check(
          "the other invocation recorded phone_lease_skipped=1",
          skippers.length === 1,
          `a=${a.phone_lease_skipped} b=${b.phone_lease_skipped}`,
        );
        check(
          "the skipper drained nothing",
          skippers[0]?.drained === 0,
          JSON.stringify(skippers[0]),
        );
        check(
          "the drain ran ONCE, not twice (no doubled MPS on the number)",
          drainCalls === 1,
          `drainCalls=${drainCalls}`,
        );
        check(
          "total sent counted once across both invocations",
          a.sent + b.sent === 1,
          `${a.sent}+${b.sent}`,
        );
      }
      ...
      console.log(
        "Case 2: leases are per-PHONE — different numbers still drain concurrently",
      );
      {
        const f = await mkFixture();
        const prov = await f.mkProvider(100_000);
        ...
        check(
          "no lease contention between different phones",
          res.phone_lease_skipped === 0,
          JSON.stringify(res),
        );
        check(
          "drains OVERLAPPED — maxInFlight 2 (per-phone, not global)",
          maxInFlight === 2,
      ...
      console.log(
        "Case 3: a cleanly-released lease is reacquirable on the next tick",
      );
      {
        const f = await mkFixture();
        const prov = await f.mkProvider(100_000);
        ...
        check(
          "lease CLEARED on clean exit (lease_until IS NULL)",
          lock?.lease_until === null,
          JSON.stringify(lock),
        );
        const r2 = await runTick(f.orgId, fake);
        check(
          "tick 2 reacquired and drained again",
          r2.drained === 1 && r2.phone_lease_skipped === 0,
          JSON.stringify(r2),
        );
      }
      ...
      console.log(
        "Case 4: an EXPIRED lease (crashed run) is reclaimed by the next tick",
      );
      {
        const f = await mkFixture();
        const prov = await f.mkProvider(100_000);
        ...
        check(
          "expired lease reclaimed — the phone drained",
          res.drained === 1,
          JSON.stringify(res),
        );
        check(
          "no skip recorded for an expired lease",
          res.phone_lease_skipped === 0,
          JSON.stringify(res),
        );
      }
      ...
      console.log(
        "Case 5: a held lease skips the phone cleanly (no error, nothing missed)",
      );
      {
        const f = await mkFixture();
        const prov = await f.mkProvider(100_000);
        ...
        `);
        let called = 0;
        const res = await runTick(f.orgId, async () => {
          called++;
          return drained(2, 0);
        });
        check(
          "the drain was NOT invoked for the leased phone",
          called === 0,
          `called=${called}`,
        );
        check(
          "run completed without error, drained 0",
          res.drained === 0 && res.sent === 0,
          JSON.stringify(res),
        );
        check(
          "skip counted (phone_lease_skipped=1)",
          res.phone_lease_skipped === 1,
          JSON.stringify(res),
        );
        ...
        `);
        check(
          "stage NOT marked missed",
          stage.schedule_missed_at === null,
          JSON.stringify(stage),
        );
        check(
          "stage NOT falsely released (sent_at still NULL)",
          stage.sent_at === null,
          JSON.stringify(stage),
        );
        ...
        `);
        check(
          "skip recorded on the lease row (skipped_count=1)",
          Number(lock.skipped_count) === 1,
          JSON.stringify(lock),
        );
        check(
          "the holder's lease was NOT stolen or cleared",
          lock.lease_until !== null,
          JSON.stringify(lock),
        );
      }
