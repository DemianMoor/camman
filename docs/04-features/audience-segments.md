...
- **`sent_from_provider_phone` preview risk on high-volume numbers.** The org's two dominant sending numbers each match roughly a third to a half of its entire contact base (462,569 and 334,150 contacts measured), so a preview scoped to one of them may approach the rules preview's 10s timeout and degrade to `truncated: true, count: null`. This is a property of the data (a handful of numbers carrying most of the org's send volume), not a defect — the underlying index-only scan itself measured well under 100ms for a selective phone (86,762 rows).
