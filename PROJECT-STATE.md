...
- **Phase 9 — Tracking IDs**: structured immutable `tracking_id` field on campaigns + stages, plus a `campaign_tracking_counters` table for atomic sequence allocation. Format: `<brand>_<offer>_<MMDDYY>_<seq>` for campaigns, `<campaign_tracking_id>_s<stage>_c<creative>` for stages. Migration 0038 adds the columns, partial unique indexes, counter table, and RLS. Reusable `<CopyableId>` component wraps the read-only surface. See [CLAUDE.md §10g](CLAUDE.md). Backfill script: `npx tsx scripts/backfill-tracking-ids.ts` — run after migration 0038 applies in any environment with pre-Phase-9 data.
