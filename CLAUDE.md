...
**`exclude_in_use_contacts` on the campaign** (`campaigns.exclude_in_use_contacts`, default **true**): the campaign-level counterpart to the per-segment flag (§10e). When on, the snapshot AND the live preview drop any contact already in another `status='active'` campaign's `campaign_audience_pool` — across the WHOLE audience (contact groups + segments), which the per-segment flag can't reach for a group-only audience. The `audience_cap` then random-samples from the remaining unused pool; when fewer unused contacts exist than the cap, all of them are sent (`min(cap, unused)`). Both flags compose (idempotent — they EXCEPT the same active-pool set). The logic lives in `buildQualifyingContactsSql` / `previewAudience` (and `computeStageAudienceCountForDraft` for draft stage previews) in [lib/audience-snapshot.ts](lib/audience-snapshot.ts); `previewAudience` still reports `in_use_in_other_campaigns` (the pre-exclusion in-use count) so the UI can show "N excluded".
