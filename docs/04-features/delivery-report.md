...
|---|---|---|---|
| A (fresh) | **today only (ET)** | every run | **1.2–3.5 s · 15,856–27,068 blocks (124–211 MB)** — five consecutive live ticks |
| B (settle) | the last 7 ET days | when the last settle is ≥ 3 h old | ~10 s |

⚠️ **Tier A covered today + yesterday until 2026-09-23**, which made this job the #3 consumer on the database — 129 runs in 21.5 h at 6.96 s and ~267 MB each, about **34 GB/day**, because every tick re-read yesterday's sends and two days of receipts.

⚠️ **The improvement is roughly 3× on time and ~1.7× on blocks — NOT the ~100× the pre-merge measurement implied.** That measurement (today+yesterday 2.6 s warm / 8.1 s cold, 12,787–44,101 blocks; today only 0.43–0.47 s, **0–2 blocks**) ran both scopes back-to-back in one session, so today's pages were already in `shared_buffers` when the today-only runs read them. Production ticks are 10 minutes apart on a 512 MB `shared_buffers` instance, and neighbouring jobs evict those pages in between — so a real tick re-reads from disk. Five consecutive live ticks after the change measured **1,182 / 3,460 / 1,298 / 1,199 / 3,462 ms** and **124 / 211 / 131 / 126 / 210 MB** (no settle among them), against the pre-change lifetime average of **6.96 s and ~267 MB per call** for the same `queryid`. The alternating high/low pattern tracks the cache state left by whatever ran in the preceding minutes, not the scope. Yesterday is not dropped — the settle tier still recomputes it every 3 h, so a late receipt for yesterday lands within 3 h instead of 10 min, and the "as of" label says so (a window including yesterday reports the settle stamp). `sent` for yesterday is unaffected either way: a send is stamped when it goes out, so yesterday's cells stop changing at midnight.
