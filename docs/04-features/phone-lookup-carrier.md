...
Live-fire drain needs `TELNYX_API_KEY` set; the first run is the 500-number calibration batch. Verified without HTTP (lease overlap/CAS/crash-recovery, attempt-summed cap, enqueue dedup — `scripts/test-lookup-worker.ts`). That test is **preview-only** (2026-09-22): it refuses every target except camman-v2, so run it with the `.env.demo` `DATABASE_URL`. It rewrites the global `lookup_settings` row, so it reads the row first, puts `worker_lease_until` back in `finally`, and fails unless the whole row reads back unchanged. **It cleans up by key, never by phone number (2026-09-22).** `phone_lookups` (PK = phone) and `lookup_queue` are global, and the test used to delete from both by fixed numbers (`+12128675309`…), so a real row carrying one of them would have been deleted. On camman-v2, the old file deleted a seeded "real" `phone_lookups` row and a `lookup_queue` row. Its numbers now come from [scripts/_fictional-phones.ts](../../scripts/_fictional-phones.ts): NANP's fictional 555-01XX range, with the area code and last two digits random per run. The run refuses to start if any of them is already in `phone_lookups`, `lookup_queue` or `contacts`. Teardown deletes only the queue rows of the batches it created, those batches by id, and the `phone_lookups` keys it inserted, then reads back 0. It makes no Telnyx request: `runLookupWorker` is called only while the test holds the lease, so it returns `no_lease` first (a dns/net trace of a full run shows no Telnyx host).
...
**`test-lookup-uploads.ts` never uses the production Telnyx key and makes no Telnyx request (2026-09-22).** `previewLookup` calls `telnyxBalance()`, a real `GET api.telnyx.com/v2/balance`. `_env-preload` loads `.env.local`, whose `TELNYX_API_KEY` is the **production** key, so until this change every run sent that key to Telnyx, while the file's header said "No Telnyx HTTP". Now, before any `lib/telnyx` module loads, the test does three things. It overwrites `TELNYX_API_KEY` with `test-not-a-real-telnyx-key`, which `client.ts` reads on every call. It drops `TELNYX_API_URL`. It wraps `fetch`: the balance GET gets canned JSON, but only when it carries the fake key, and any other Telnyx request is recorded and throws. Bars assert three things: the balance `previewLookup` returned is the canned value, exactly one Telnyx request was attempted, and there was no unexpected one. A dns/net trace of the run shows no Telnyx host. The test also cleans up by key: `importCsvLookups` upserts on phone, so its old fixed numbers (`+12122000001`…) would have overwritten a real row and then deleted it. On camman-v2 the old file did exactly that to a seeded row, and still went green. It now uses `_fictional-phones`, the same refusal, and deletes only the `phone_lookups` keys it inserted.
...
- **Perf / caching:** the aggregate is a ~2s full-population scan (750K+ contacts, opt-outs pre-hashed to a set like the audience builder), too heavy per page view. Cached per-org in `lookup_group_stats_cache` (migration 0106); the panel reads it in ~46ms with a prominent "as of ET" label + "may be stale" badge past the 15-min TTL, and a manager-only "Refresh now". **Fail-safe:** `refreshLookupGroupStats` computes FIRST, then does one atomic upsert of the whole blob — a failed recompute (query error or broken invariant) leaves the prior cache intact and returns 500, so the panel degrades to older data, never blank. `GET /api/telnyx/lookup/group-stats` (read/first-compute) + `POST …/refresh` (force), both manager+. Verified: `scripts/test-lookup-stats.ts`. **That test is preview-only and builds its own world (2026-09-22).** It used to load `.env.local` itself, so a bare run read **production** and overwrote the first org's real `lookup_group_stats_cache` row. Its only bars were the reconciliation identities, which hold vacuously over an empty org. Now it has the `_env-preload` + `_require-preview-db` guard on lines 1–2. It seeds a throwaway org whose name carries `__LOOKUP_STATS_TEST__`: two active groups and one archived, a landline, a contact in two groups, and a duplicate opt-out. It then asserts the **exact** per-group and summary numbers that world produces. Teardown deletes the `phone_lookups` keys it inserted (numbers from `_fictional-phones`) and the org, which cascades the rest, then reads back 0. Its cache write happens inside the library, so it is named in `GUARDED_VIA_LIBRARY` in `check:guards` ([07-conventions.md](../07-conventions.md)).
