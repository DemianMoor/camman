...
// a send-eligible state ('draft' | 'pending' | 'sent') with NO
// provider_phone_id resolves its key via the single-credential legacy
// fallback (see resolveKeyForStage) — which goes ambiguous the moment the
// provider gains a 2nd credential, since the resolver can no longer guess
// which account to bill. The POST route calls this before inserting a 2nd+
// credential and blocks (409) when it returns > 0.
