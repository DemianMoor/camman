...
export async function runStageDrain(
  ...
  const ctx = (await dbc.execute(sql`
    SELECT s.sms_provider_id AS provider_id,
           ...
           --
           -- No COALESCE fallback. It existed only to make the 0134 cutover a
           -- no-op by construction and was removed with the txh2 alias, after
           -- 40,120 live txh2 sends confirmed resolution via adapter_code.
           -- A NULL adapter_code on an api-send row is a DATA ERROR (migration
           -- 0135 guards against creating one), and a hard unknown_provider
           -- refusal surfaces it — a silent fallback would mask it.
