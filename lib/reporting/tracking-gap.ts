...
// Imported (not just re-exported below) because the query + report builder in
// this file USE them — `export { x } from "…"` does not bind x locally.
import {
  TRACKING_GAP_MATURITY_HOURS,
  TRACKING_GAP_MIN_HUMAN_CLICKS,
  TRACKING_GAP_WINDOW_DAYS,
  trackingGapBreached,
} from "./tracking-gap-rules";
...
// The pure predicates + their calibration notes live in tracking-gap-rules.ts
// (no db/drizzle imports) so the client campaign page can share them. Re-
...
export type { ClickerSubstitutionInput } from "./tracking-gap-rules";
