...
- **Purchase rules read `stage_sends.sale_status`** — a contact matches when they have ≥1 `stage_sends` row carrying a **non-rejected conversion**, i.e. `sale_status IN ('lead','sale')`. The single source of truth is `purchasedClause()` in [`lib/sale-attribution.ts`](../../lib/sale-attribution.ts); do not inline the predicate. **`'lead'` counts**: `sale_status` stores the network's RAW Keitaro postback status, and this account's network fires `lead` for PAID conversions (it effectively never sends `sale` — all 852 `lead` rows carry payout revenue, avg $67). `'rejected'` (refund/chargeback) does **not** count. `sale_status` is stamped per-recipient by the Keitaro conversions poll ([`lib/keitaro/poll-conversions.ts`](../../lib/keitaro/poll-conversions.ts)) via `sub_id_1` → `stage_sends.id`. Brand/offer scoping joins `stage_sends → campaigns` (brand/offer live on the campaign, not the send). Both operators supported: `is` (bought) / `is_not` (didn't buy). **Empty until real sales accumulate** — with no sales, the rule resolves to manual membership only (an empty preview means "no buyers yet", not a bug). This is engagement **Level 3**.
- **One definition of a sale, org-wide.** The purchase rules, the [behavioural lanes](behavioral-lanes.md) converted tier, and the reporting surfaces ([`lib/keitaro/poll.ts`](../../lib/keitaro/poll.ts) counts every conversion row; [`lib/reporting/rollup.ts`](../../lib/reporting/rollup.ts) uses `converted_at IS NOT NULL`) must all agree. They did not until 2026-08-19: the rules tested `sale_status = 'sale'` and so found **2** buyers where the reports showed **835**. Guarded by [`scripts/verify-purchase-rule-definition.ts`](../../scripts/verify-purchase-rule-definition.ts), which asserts the rule count equals the live non-rejected-conversion count rather than a frozen number.
...
- `segment_stats.rule_filtered_count` (on-demand via `/api/segments/[id]/refresh-stats`) = the **SENDABLE** audience count — the full UNION'd audience (manual ∪ rule matches) **minus opt-outs**; NULL when no active rules or the eval timed out. Name is historical — it's really `sendable_audience_count`.
- **Opt-outs are excluded from every segment-page audience read (changed 2026-08-17):** the header tile, the Audience tab (list *and* counts), the rules preview, and the "Export contacts" CSV. Previously they were counted, so a segment advertised an audience no campaign could ever draw — in the reported case 54,356 on the segment page versus 44,480 as a campaign's "From segments", the gap being exactly its 9,876 opt-outs. The campaign path is deliberately **not** changed: it keeps opt-outs in the source set so `previewAudience` can report `excluded_for_optout` before stripping them, and the snapshot already anti-joins them once against a temp table with real row stats. The shared wrapper is `excludeOptOutsFromAudience()` in [`lib/segment-rules-eval.ts`](../../lib/segment-rules-eval.ts) — segment-page read paths only. Export excludes them for compliance, not just arithmetic: that CSV is uploaded straight into a provider tool.
- **Engagement counters are audience-based (changed 2026-08-17).** `opt_out_count` / `opt_in_count` / `clicker_count` are computed over the **full** audience (pre-opt-out-suppression), on one basis, so the page reconciles: `rule_filtered_count + opt_out_count = full audience`. They previously counted only manual `segment_contacts` membership, which is empty for most rule-based segments — a segment named "Clickers excl Buyers" reported `Clickers 0` and `Opt-Outs 0` while 9,876 of its audience were suppressed, which is why the loss was invisible. Segments with no active rules keep the manual-membership figures (the audience *is* the manual membership there), and a timed-out audience query falls back to them rather than persisting zeros — the columns are `NOT NULL`.
- **Preview:** `POST /api/segments/[id]/rules/preview` → `{ count, manual_count, rule_filtered_count, duration_ms, truncated }`. Hard **10s** `SET LOCAL statement_timeout` inside a transaction; on timeout (PG `57014`) returns `truncated:true, count:null` rather than 500 (`previewSegmentAudienceCount()`). That helper returns the counters in **one pass** over the audience, hash-joining deduped status CTEs rather than probing per row with correlated `EXISTS` (same shape as the campaign preview's `flagSetCtes`/`flagJoins`).
...
- Campaign audience snapshots respect a segment's internal UNION semantics ([audience-snapshot.md](audience-snapshot.md)) but **frozen pools are NOT recomputed** when rules change later — by design.
- **UNION lives *inside* a segment, not across them.** Rules within one segment can `or` together; the campaign's selected include-segments **INTERSECT** (changed 2026-08-17 — see [audience-snapshot.md §3](audience-snapshot.md)). So "clickers OR opt-ins" is one segment with two `or` rules, not two segments on a campaign. Beware the interaction with filter-shaped segments: a lone `is_not` rule matches nearly the whole org, which is harmless as an AND-ed narrowing filter and catastrophic as an OR-ed branch.
