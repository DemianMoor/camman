...
| **Telegram Bot API** | app → alerts + reports | circuit-breaker / poller alerts (best-effort) + hourly/daily performance report (`/api/cron/telegram-report`) | bot token + chat id | `sendMessage` — best-effort for alerts (`notifyTelegram`), `parse_mode:"HTML"` + error-propagating for the report (`sendTelegramHtml`). ⚠️ **`sendMessage` 400s both on malformed markup and on text over 4096 chars, and the report treats a 400 as `permanent`** — so a bad message 500s the cron every hour until a human edits a row. The report escapes every user-typed string (`event_types.label` has no CHECK and no UI) and caps the assembled, post-escape message at 3500 chars, dropping whole lines so an HTML entity is never cut in half. See [conventions](07-conventions.md) |
...
**Per-number settings come from the descriptor.** A connection type declares `phoneSettingFields` (name, label, placeholder, help, `required`); `name` must match the `provider_phones` column the input binds to. `required: true` is enforced in three places off the one flag — the form's red asterisk, the phones POST (missing ⇒ 400 `required_provider_setting_missing`) and the phones PATCH (clearing ⇒ the same 400). The provider detail response resolves them from `adapter_code` and returns `phone_setting_fields`, so the phone form renders them without importing the registry. Text Request declares `dashboard_id`; no other type declares any today. This replaced a hardcoded `providerKey === "txr"` gate that keyed on row identity — under it a second account of a type would not have received its type's fields.
