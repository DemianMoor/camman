...
- **Total Cost auto-derivation** (migration `0081`, [`lib/stages/total-cost.ts`](../../lib/stages/total-cost.ts)): a stage's `total_cost` defaults to `cost_per_sms × (sms_count + opt_out_count)` — the assigned provider phone's per-SMS rate times sends **plus** opt-out replies (STOPs are billed like sends). It's recomputed on every write that moves those inputs: the manual-results save, the opt-out poller after a STOP bumps `opt_out_count`, and the stage PATCH when `provider_phone_id` changes. `total_cost_manual` is the override flag: the manual-results form's **Auto-calculate total cost** switch turns it on/off, and a CSV import that carries a real provider cost sets it (the imported figure is authoritative and replaces a previously auto-derived value). When the flag is true the auto formula never touches `total_cost`.
