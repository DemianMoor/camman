...
- **Offer landing pages** (`offer_landing_pages`, 0150) are managed in the offer's **Edit** dialog by [`components/offers/landing-pages-panel.tsx`](../../components/offers/landing-pages-panel.tsx): add, make default, disable/enable, and **edit title / kind / slug / URL in place** (pencil on the row). No delete. Editing the destination of a page that unsent or dripping stages still use returns `409 landing_page_in_use` with the counts; the panel shows a confirm dialog and resends with `confirm: true`. Switching to a slug page is refused while an affected stage's brand has no landing host. Permission: `offers.update` (manager+); the single-page route stays denied to operators. Rules in [07-conventions.md](../07-conventions.md) → "Stage destinations are built at MINT time".
