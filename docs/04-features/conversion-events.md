...
- **Deliberate trades, all three checked by `scripts/test-stage-day-conversions.ts`:**
  ...
  - The **Checkout Clicks mirror follows the tracker only where the tracker owns the counter**: a stage's `checkout_click_count` is mirrored exactly (down to 0) only while it still equals that stage's Keitaro checkout sum from BEFORE the run. Any other value was entered by hand, and a tracker 0 never overwrites it (a positive tracker sum still does — the long-standing guarded behaviour). From the Phase 3 deploy until the 2026-09-21 fix the projection mirrored every in-scope stage exactly and zeroed hand-entered values (stage 130: 22 → 0) — see [keitaro-poll.md §2a](keitaro-poll.md). `scripts/resync-stage-day-conversions.ts --apply` follows the same rule for every stage.
...
— **124/0** on camman-v2 inside a transaction that always rolls back (70 before 0185, 108 before the resync bars, 119 before `main`'s H1–H5 arrived with the 2026-09-21 merge).
