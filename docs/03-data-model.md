...
| `offer_report_campaign_econ` (plain **view**) | `campaign_id`, `org_id`, `offer_id`, `group_ids int[]` (= `campaigns.audience_contact_group_ids`), `sends`, `revenue numeric(12,4)`, `sales`, `clicks`, `cost numeric(12,4)`, `optouts`, `has_manual_stages` | per-campaign economics for every campaign of an offer with ≥1 sent stage (tracked **and** manual); sole source of the offer total (`offer_report_offer_totals_mv`) and org benchmark rows — see the locked metric definitions in [04-features/offer-group-report.md](04-features/offer-group-report.md) |
